...
void Fingerprint_EVENT_JSON ( struct _DecodeAlert *DecodeAlert, struct _FingerprintData *FingerprintData, char *str, size_t size )
{
    ...
    if ( !strcmp(DecodeAlert->app_proto, "http") )
        {
	      ...
	      strlcpy(http, json_object_to_json_string_ext(encode_json_http, JSON_C_TO_STRING_NOSLASHESCAPE), sizeof(http));

        }


    strlcpy(fp, json_object_to_json_string_ext(encode_json_fp, JSON_C_TO_STRING_NOSLASHESCAPE), sizeof(fp));
    ...
    snprintf(tmp_string2, sizeof(tmp_string2), "%s \"fingerprint\": %s}\0", tmp_string, fp); 
    strlcpy(tmp_string, tmp_string2, sizeof(tmp_string));

      if ( http[0] != '\0' )
	{
	tmp_string[ strlen(tmp_string) - 1 ] = ',';
	snprintf(tmp_string2, sizeof(tmp_string2), "%s \"http\": %s}\0", tmp_string, http);
	}
